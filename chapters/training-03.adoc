...
==== Mengimplementasikan __Handler__
...
----
func writeJSON(w http.ResponseWriter, status int, payload interface{}) { //<1>
	w.WriteHeader(status)
...
func (s *HTTPServer) NewArticleHandler(w http.ResponseWriter, r *http.Request) {
	...
	article, err := s.uc.CreateArticle(ctx, payload.Title, payload.Content) // <3>
...
----
<1> Fungsi utilitas untuk menulis JSON sebagai respons. 
<2> __Payload__ yang kita terima dari __request__.
<3> Jalankan __behaviour__ dari Use Case.
<4> Kembalikan hanya hasil yang berupa side effect.
