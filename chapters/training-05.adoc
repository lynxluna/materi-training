...
Kita buat konstruktor buat `SQLStore` dengan mengisi `db`:
...
func CreateSQLStore(driver, connString string) (*SQLStore, error) { // <2>
...
Kemudian kita implementasikan `FindArticleByID`:
...
func (s *SQLStore) SaveArticle(ctx context.Context, article Article) error {
	...
	_, err = squirrel.
		...
		SetMap(updateMap).RunWith(s.db).ExecContext(ctx) // <2>

	if err == nil { // <3>
		return err
	}

	idPredicate := squirrel.Eq{"id": article.ID} // <4>
