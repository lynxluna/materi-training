...
> Go is an attempt to combine the safety and performance of statically typed languages with the
> convenience and fun of dynamically typed interpretative languages. -- Rob Pike
...
. Buat program dengan Go untuk menampilkan `n` bilangan fibonacci pertama.

. Buat program dengan Go untuk kasus __stack__ dan __queue__ menggunakan array atau slice. Gunakan
fungsi `push` dan `pop` untuk __stack__ dan `enqueue` dan `dequeue` untuk __queue__.
